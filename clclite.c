...
void scroll_buffer (char buffer[][MAX_LINE_LENGTH])
{
	...
	for (y = 1; y < MAXLINES; y++)
		strcpy (buffer[y - 1], buffer[y]);
...
int add_line_buffer (char buffer[][MAX_LINE_LENGTH], char *line, int lastline)
{
	...
	if (strlen (line) > (MAX_LINE_LENGTH - 2))
		line[MAX_LINE_LENGTH - 2] = '\0'; /* Don't go off the end! */
...
int add_char_buffer (char buffer[][MAX_LINE_LENGTH], char c, int lastline)
{
	...
	if (x > (MAX_LINE_LENGTH - 2))
		x = MAX_LINE_LENGTH - 2; /* Don't go off the end! */
...
static void on_text_ansi (const char *text, size_t len)
{
	...
	for (i = 0; i < len; ++i)
	{
		if (text[i] == '\007')
		{
			beep();    /* If we have an incoming bell, play it and skip it*/
			continue;
		}
